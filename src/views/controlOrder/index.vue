...
<div>
  <div class='header'>
    <x-header style='background:#2d8cf0;' :left-options="{showBack: false}">监控中的工单<a slot="right" v-on:click="showMenus = true">筛选</a></x-header>
    <search
...
import {wocontrol} from '../../api/request'
...
export default {
  ...
  methods: {
    initData(){
      wocontrol(this.params).then(res=>{
...
this.$refs.scrollerBottom.disablePullup()
 ...
 async getData() {
   ...
   var arr =[]
  await wocontrol(this.params).then(res => {
    if (res.data.code === 0) {
