  ...
  <div>
    <sheader :title="titleMsg"></sheader>
     <group    label-width="6em" label-margin-right="2em" label-align="justify">
      ...
      <x-textarea title="审核原因" placeholder="请输入内容" :show-counter="false" :rows="3" v-model="params.text"></x-textarea>
...
import {submitWorkOrder} from '../../api/request'
export default {
  components: {
    sheader,
    Group,
  ...
  methods: {
    submit(){
      if( this.params.text === ''){
         ...
         submitWorkOrder(this.params).then(res => {
          if (res.data.code === 0) {
            this.replymsg = true;
